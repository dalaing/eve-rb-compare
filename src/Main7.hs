...
networkDescription (Sources sc st) = do
  ...
  bPos <- stepper 0 ePos
  let bPosStatus = (\p -> "Pos: " ++ show p) <$> bPos
  ...
  let bScoreStatus = (\s -> "Score: " ++ show s) <$> bScore

  let
    bStati = sequenceA [bPosStatus, bScoreStatus]
    bRender = toString <$> bPos <*> bTreasure <*> bStati
